...
// Carrega a tabela de produtos do supermercado
async function loadProductTable() {
  try {
    ...
    let html = "";
    products.forEach((p) => {
      // 1. IMAGEM: Adicionado 'object-fit: cover' para imagens não ficarem distorcidas e um fallback caso a imagem falhe
      const imgSrc = p.image ? p.image : 'https://via.placeholder.com/50x50?text=Sem+Foto';
      const img = `<img src="${imgSrc}" onerror="this.src='https://via.placeholder.com/50x50?text=Erro'" class="rounded-2 shadow-sm me-3" style="width: 45px; height: 45px; object-fit: cover; border: 1px solid #eee;">`;
  ...
  swalCustom
    ...
    `,
      ...
      preConfirm: () => {
        ...
        formData.append("categoryId", document.getElementById("add-category").value);
    ...
    swalCustom
      .fire({
        ...
        <div class="text-start mt-3">
          ...
          <div class="mb-3">
            <label class="form-label-small">Imagem do Produto</label>
            <input type="file" id="edit-image" accept="image/*" class="form-control w-100">
          </div>
          <div class="row">
            <div class="col-6 mb-3">
              <label class="form-label-small">Preço (€)</label>
