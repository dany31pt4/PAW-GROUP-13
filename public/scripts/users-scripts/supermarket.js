    ...
    preConfirm: () => {
      ...
      if (!email || !password || !name || !address) {
        Swal.showValidationMessage(
          "Nome, Email, Password e Morada são obrigatórios!",
        );
        return false;
      }
      ...
      fetch("/api/supermarkets/create", {
        ...
        .then(async (res) => {
          ...
          if (res.ok && data.success) {
            ...
            Swal.fire(
              "Erro!",
              data.message || "Erro ao criar o supermercado.",
              "error",
            );
          }
        })
...
async function openViewMarketModal(marketId) {
  ...
  try {
    const res = await fetch(`/api/supermarkets/${marketId}`);
    const market = await res.json();

    let email = '<span class="text-muted">Desconhecido</span>';
    let phone = '<span class="text-muted">Sem contacto</span>';

    if (market.user) {
      email = market.user.email;
      phone = market.user.phone;
    }
    ...
    Swal.fire(
      "Erro",
      "Não foi possível carregar os dados do supermercado.",
      "error",
    );
  }
}
...
async function openEditMarketModal(marketId) {
  ...
  try {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const updateRes = await fetch(`/api/supermarkets/update/${marketId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(result.value),
        });
        if (updateRes.ok) {
          Swal.fire("Sucesso!", "Dados atualizados.", "success").then(() =>
            loadMarketTable(),
          );
        } else {
...
function openRemoveMarketModal(marketId) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        const delRes = await fetch(`/api/supermarkets/delete/${marketId}`, {
          method: "DELETE",
        });
        ...
        if (delRes.ok && data.success) {
          Swal.fire(
            "Eliminado!",
            "O supermercado e a respetiva conta foram removidos.",
            "success",
          ).then(() => loadMarketTable());
        } else {
          Swal.fire(
...
// 5. ATUALIZAR TABELA DE SUPERMERCADOS 
async function loadMarketTable() {
  try {
    const response = await fetch("/api/supermarkets/list");
    ...
    if (!tbody) {
      console.error(
        "Aviso: Tabela de supermercados não encontrada nesta página.",
      );
      return;
    }
